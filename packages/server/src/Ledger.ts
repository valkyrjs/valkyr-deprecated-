...
export class Ledger {
  ...
  public get collection() {
    return this.server.collection<Event>("events");
  }
   ...
   */
  public async insert(event: Event) {
   ...
   */
  public async outdated({ streamId, type, created }: Event): Promise<boolean> {
  ...
  public async events(created?: string, direction?: 1 | -1) {
    ...
    return this.collection.find(filter).sort("created").toArray();
  }
  ...
  public async stream(streamId: string, created?: string, direction: 1 | -1 = 1) {
    ...
    return this.collection.find(filter).sort({ created: direction }).toArray();
  }
  ...
  public async pull(streamId: string, recorded?: string) {
    ...
    return this.collection.find(filter).sort({ recorded: 1 }).toArray();
  }
}

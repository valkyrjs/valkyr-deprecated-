...
export class IdentityStorageService {
  ...
  async export(id: string, access: AccessKey): Promise<void> {
    await this.remote.post<IdentityStorageSchema>("/identities", await this.#getExportPayload(id, access));
  }

  async update(id: string, access: AccessKey) {
    await this.remote.put<IdentityStorageSchema>("/identities", await this.#getExportPayload(id, access));
  }
  ...
  async #getExportPayload(id: string, access: AccessKey) {

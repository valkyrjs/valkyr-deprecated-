...
class EventStore<Record extends EventRecord = EventRecord> {
  readonly inserted = new Subject<{ record: Record; hydrated: boolean }>();

  constructor(
    readonly collection: Collection<Document<Record>>,
   ...
   */
  async push(container: string, stream: string, event: ExtractEvent<Record>): Promise<void> {
    const record = createEventRecord(container, stream, event);
  ...
  async events(cursor?: string, direction?: 1 | -1) {
    ...
    return this.collection.find(filter, { sort: { created: 1 } });
  }
...
export const eventStore = new EventStore<AppEventRecord>(db.collection("events"));

eventStore.inserted.subscribe(async ({ record, hydrated }) => {
  if (hydrated === true) {
    return; // don't emit hydrated events
  }
  const { error } = await supabase.from("events").insert({
    ...record,
    recorded: undefined // remove local timestamp
  });
  if (error) {
    console.log(error);
  }
